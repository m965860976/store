...
con = 0
# 随机生成优惠券
lis = [0.9, 0.9, 0.9, 0.9, 0.9, 0.9, 0.9, 0.9, 0.9, 0.9, 0.5, 0.5, 0.5, 0.5, 0.5, 0.1, 0.1, 0]
favo = random.choice(lis)
print(favo)
merch = random.randint(0, 8)
print(merch)
# 购物车
mycart = []
# 展示商品
    ...
    if chose.isdigit():  # str判断引号内是否为数字
        ...
        if chose < len(shop):  # 判断输入的内容是否小于列表的长度
            if money > shop[chose][1]:  # 判断money是否大于商品的价格
                ...
                if chose == merch:
                    con = con + shop[chose][1] * favo
                else:
                    con = con + shop[chose][1]
                print("恭喜你成功加入购物车,合计：", con)
            else:
                print("gun")
        else:
            print("没有此商品")
